...
void tableau::MoveDiagonal(int x, int y, destination destinations[maxpos], int &N_possible, int steps)
{
  ...
  if(steps < 0)
    {
      test_dest[i].x=x-min(x-0,y-0); test_dest[i].y=y-min(x-0,y-0); i++;
      test_dest[i].x=x+min(5-x,y-0); test_dest[i].y=y-min(5-x,y-0); i++;
      test_dest[i].x=x+min(5-x,5-y); test_dest[i].y=y+min(5-x,5-y); i++;
      test_dest[i].x=x-min(x-0,5-y); test_dest[i].y=y+min(x-0,5-y); i++;
    }
  else
    {
      test_dest[i].x=x-steps; test_dest[i].y=y-steps; i++;
      test_dest[i].x=x+steps; test_dest[i].y=y-steps; i++;
      test_dest[i].x=x+steps; test_dest[i].y=y+steps; i++;
      test_dest[i].x=x-steps; test_dest[i].y=y+steps; i++;
...
void tableau::MoveStraight(int x, int y, destination destinations[maxpos], int &N_possible, int steps)
{
  ...
  if(steps < 0)
    {
      test_dest[i].x=0; test_dest[i].y=y; i++;
      test_dest[i].x=5; test_dest[i].y=y; i++;
      test_dest[i].x=x; test_dest[i].y=0; i++;
      test_dest[i].x=x; test_dest[i].y=5; i++;
    }
  else
    {
      test_dest[i].x=x+steps; test_dest[i].y=y; i++;
      test_dest[i].x=x-steps; test_dest[i].y=y; i++;
      test_dest[i].x=x; test_dest[i].y=y+steps; i++;
      test_dest[i].x=x; test_dest[i].y=y-steps; i++;
      //If off-limits, return to initial pos
      for(i=0;i<movepos;i++)
	{
